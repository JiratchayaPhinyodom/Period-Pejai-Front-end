...
useEffect(() => {
    try {
        const url_diary = 'http://creammmm.pythonanywhere.com/api/diary' + '?uid=' + currentUser.uid
...
const setR = useCallback((data) => {
    ...
    // [[],[],[]] not use
    // [[]] use this
    let url = "http://creammmm.pythonanywhere.com/api/period";
    fetch(url, {
        method: "POST",
        ...
        const url = 'http://creammmm.pythonanywhere.com/api/predict' + '?uid=' + currentUser.uid
        const url2 = 'http://creammmm.pythonanywhere.com/api/period' + '?uid=' + currentUser.uid
        const url_luteal = 'http://creammmm.pythonanywhere.com/api/luteal' + '?uid=' + currentUser.uid
        // const res = await fetch('https://pokeapi.co/api/v2/pokemon/ditto')
